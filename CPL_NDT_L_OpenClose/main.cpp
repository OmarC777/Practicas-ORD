...
void menuUniversal(EstructurasDatos* estructuras) {
    ...
    do {
        ...
        switch(opcion) {
            ...
            case 3: estructuras->mostrar(); break;
            case 4: 
                if (estructuras->estaLlena()) cout << "Estado: SI esta llena.\n";
                else cout << "Estado: NO esta llena (dinamica).\n";
                break;
            case 5: 
            ...
            case 6: 
                ...
                break;
            case 7: cout << "Regresando al panel principal...\n"; break;
            default: cout << "Opcion no valida. Por favor intenta de nuevo.\n"; break;
        }
    } while (opcion != 7);
}
int main(int argc, char** argv) 
{
	NavegadorEstructura obNav;
    RecepcionHotel obHotel;
    PapeleriaLista obPape; 
    
    vector<EstructurasDatos*> estructuras = {&obNav, &obHotel, &obPape};
    
    int opcionP;
    do {
        
        cout << "MENU DE ESTRUCTIRAS" << endl;
        for (size_t i = 0; i < estructuras.size(); i++) {
            cout << i + 1 << ". Entrar a " << estructuras[i]->getNombre() << endl;
        }
